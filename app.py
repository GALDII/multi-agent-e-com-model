...
with st.sidebar:
    ...
    # Input 2: Price API Key (NEW)
    price_api_key = st.text_input("Enter your Price API Key (Optional)", type="password")
    ...
    if st.button("🚀 Run Analysis"):
        ...
        if not api_key:
            ...
            if st.session_state.raw_data.empty:
                ...
                # --- Run Agent 2: Analyst ---
                # We pass the price_api_key here now
                with st.spinner("📈 Agent 2: Analyzing and tracking prices..."):
...
if not st.session_state.ran_analysis:
    st.info("Enter your API keys and a product query in the sidebar to start the analysis.")
else:
    ...
    # --- Tab 2: Historic Reports (NEW) ---
    with tab_history:
        st.header("Historical Price Analysis")
        st.info("Agent 2 used the Price API to check if today's price is lower than the historical average.")
        if not st.session_state.historic_report_df.empty:
            ...
            st.warning("No historic data available (Check API Key or Product Links).")

    # --- Tab 3: Potential Deals ---
    with tab_deals:
        st.header("Potential Deals (AI Prediction)")
